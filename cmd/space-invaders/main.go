...
func main() {
	...
	if err != nil {
		logger.Fatal("Failed to read public key", zapcore.Field{Key: "error", Interface: err, Type: zapcore.ErrorType})
	}

	key, err := jwt.ParseRSAPublicKeyFromPEM(raw)
	if err != nil {
		logger.Fatal("Failed to parse public key", zapcore.Field{Key: "error", Interface: err, Type: zapcore.ErrorType})
	}
